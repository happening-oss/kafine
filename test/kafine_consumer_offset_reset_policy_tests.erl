...
earliest() ->
    ...
    Partition = 0,
    Subscription = #{TopicName => {#{offset_reset_policy => earliest}, #{Partition => -1}}},
...
latest() ->
    ...
    TopicName = ?TOPIC_NAME,
    Partition = 0,
    Subscription = #{TopicName => {#{offset_reset_policy => latest}, #{Partition => -1}}},
    kafine_consumer:subscribe(Consumer, Subscription),
...
last() ->
    ...
    % If we specify 'last', we should initially get only the most recent message.
    TopicName = ?TOPIC_NAME,
    Partition = 0,
    Subscription = #{
        TopicName => {#{offset_reset_policy => kafine_last_offset_reset_policy}, #{Partition => -1}}
    },
    kafine_consumer:subscribe(Consumer, Subscription),
